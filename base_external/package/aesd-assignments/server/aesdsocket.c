...
printf("My name is bond2");
  //  openlog("aesdsocket", LOG_PID, LOG_USER);
    ...
    if (server_socket == -1) {
        ...
        printf("My name is bond4");
        exit(1);
    }

    if (bind(server_socket, res->ai_addr, res->ai_addrlen) == -1) {
       // syslog(LOG_ERR, "Bind failed");
       printf("My name is bond5");
        close(server_socket);
        exit(1);
    }
